...
export default function ActionsScreen() {
  ...
  const renderActionBlocks = ({ item }: { item: ActionBlock | null }) => {
    if (item) {
      ...
      return (
        ...
        >
          <TouchableOpacity
            style={[styles.itemContent, { height: '100%', overflow: 'hidden' }]}
            onPress={() => {
      ...
      if (statsData.level >= 3) {
        ...
        if (spell) {
          raceActions.push({
            ...
            details: 'Cast as SpLv2 (3d10). Finish a long rest to cast again.',
            type: 'spell',
            source: 'race',
          } as ActionBlock);
        }
      ...
      if (statsData.level >= 5) {
        ...
        if (spell) {
          raceActions.push({
            ...
            details: 'Finish a long rest to cast again.',
            type: 'spell',
            source: 'race',
          } as ActionBlock);
        }
    ...
    if (statsData.class?.toLowerCase() === 'cleric') {
      ...
      if (subclass?.toLowerCase() === 'order' && orderDomainEnabled === true) {
        ...
        classActions.push({
          ...
          cost: { actions: 0, bonus: 0 },
          details: 'If you cast a level 1 spell or higher, that targets an ally, that ally can use their reaction immediately after the spell to make one weapon attack against a creature of your choice that you can see.',
  ...
  const renderSpellDetails = (spellName: string) => {
    ...
    const lowerCaseSpellName = spellName.toLowerCase();
                  ...
                  <View>
                    ...
                    <View style={{ flexDirection: 'row', gap: 10 }}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        {('type' in selectedAction && selectedAction.type === 'cantrip') &&
                          <View style={{ marginBottom: 20 }}>
                            <Text style={{ fontStyle: 'italic', color: 'black' }}>
                              School: {cantripsData.find(c => c.name.toLowerCase() === selectedAction.name.toLowerCase())?.school}
                            </Text>
                            <Text style={{ fontStyle: 'italic', color: 'black' }}>
                              Duration: {cantripsData.find(c => c.name.toLowerCase() === selectedAction.name.toLowerCase())?.duration}
                            </Text>
                            <Text style={{ fontStyle: 'italic', color: 'black' }}>
                              Range: {cantripsData.find(c => c.name.toLowerCase() === selectedAction.name.toLowerCase())?.range}
                            </Text>
                            <Text style={{ fontStyle: 'italic', color: 'black' }}>
                              Components: {cantripsData.find(c => c.name.toLowerCase() === selectedAction.name.toLowerCase())?.components}
                            </Text>
                          </View>
                        }
                        ...
                        {(
                          selectedAction.name.toLowerCase() === 'jump' ||
                          selectedAction.name.toLowerCase() === 'unarmored movement' ||
                          selectedAction.name.toLowerCase() === 'voice of authority' ||
                          selectedAction.name.toLowerCase() === 'orders wrath' ||
                          selectedAction.name.toLowerCase() === 'fighting style'
                        ) && (
                            <Text style={{ fontStyle: 'italic', marginBottom: 5, color: 'black' }}>
                              (Passive)
                            </Text>
                    ...
                    {selectedAction.name.toLowerCase() === 'hellish rebuke' && (
                      renderSpellDetails(selectedAction.name)
                    )}
                    {/* Darkness Details */}
                    {selectedAction.name.toLowerCase() === 'darkness' && (
                      renderSpellDetails(selectedAction.name)
